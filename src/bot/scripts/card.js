...
const card = new Command({
  ...
  cb: async function (client, message, guild) {
    ...
    let cardname = message.content.slice(prefix.length + "card ".length)

    rp("https://sppd.feinwaru.com/api/v1/cards?name=" + cardname)
      .then(async response0 => {

        const firstRp = JSON.parse(response0);

        let cardresponse = firstRp.data.cards[0]

        let id = cardresponse._id

        rp("https://sppd.feinwaru.com/api/v1/cards/" + id)
