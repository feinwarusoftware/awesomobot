...
const card = new Command({
  ...
  // -card <name>
  cb: async function (client, message) {
    ...
    rp("https://sppd.feinwaru.com/api/v1/cards/list")
      .then(async response => {
        ...
        await readDir(directoryPath)
          .then(async files => {
            ...
            if (result) {
              async function downloadImage(pdfURL, outputFilename) {
        ...
        rp(`https://sppd.feinwaru.com/api/v1/cards/${cardId}`)
          .then(async response1 => {
            ...
            function _calculateCardAugmentData(original, utype, uvalue) {
              ...
              const alteredStats = card.tech_tree.slots
                  ...
                  if (k.startsWith("stat_")) {
                    if (a[k.slice(5)] != null) {
                      ...
                      return console.error(
                        "error applying upgrade stats 1: " + k
                      );
                    }
                    ...
                    if (powerIndex !== -1) {
                      ...
                      if (k === "power_duration") {
                        ...
                        return console.error(
                          "error applying upgrade stats 2: " + k
                        );
                      }
                    }
                  } else {
            ...
            bg.write(
              path.join(__dirname, "temp", `pd-${saveDate}.png`),
              async () => {
                ...
                let powers = [];
                if (
                  (cardData.powers == null ? 0 : cardData.powers.length) !== 0
                ) {
                  ...
                  cardData.powers.forEach(e => {
                    ...
                    if (e.type !== null) {
                      embed.description += `Power Type: ${removeUnderscores(e.type)} \nPower Amount: ${e.amount}\n`;
                    }
                ...
                if (
                  ...
                  ) / 100} at ${Math.round(cardData.knockback_angle * 100) /
                    100}°\n\n`;
                } else {
      ...
      .catch(error => {
        ...
        const discordEmbed = new discord.RichEmbed()
          .setColor("#F8534F")
          .setTitle(":x: Error 2. Please contact the developer");
        message.channel.send(discordEmbed);
      });
  },
