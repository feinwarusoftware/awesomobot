...
router.route("/").get(authUser, (req, res) => {
  ...
  // + cond discord data
  const current = req.query.extended === "true";
  const sort_dir = req.query.reversed === "true" ? 1 : -1;
  const sort = {
    ... req.query.order === undefined
      ? { [req.query.order]: -1 }
      : { [req.query.order]: sort_dir }
  };
  ...
  promises.push(schemas.UserSchema
    .find(search)
    .skip(page * limit)
    .limit(limit)
    .sort(sort)
  ...
  Promise.all(promises).then(data => {
    ...
    return res.json({
      ...
      total: data[0],
      users: data[1],
      ... current === false ? {} : { current: req.user }  }
    );
...
router.route("/:discord_id").get(authUser, (req, res) => {
  ...
  req.user.banner = req.body.banner === undefined
    ? req.user.banner
    : req.body.banner;
  req.user.bio = req.body.bio === undefined
    ? req.user.bio
    : req.body.bio;
  req.user.socials = req.body.socials === undefined
    ? req.user.socials
    : req.body.socials;
  req.user.modules = req.body.modules === undefined
    ? req.user.modules
    : req.body.modules;

  if (req.user.admin === true) {

    req.user.admin = req.body.admin === undefined
      ? req.user.admin
      : req.body.admin;
    req.user.verified = req.body.verified === undefined
      ? req.user.verified
      : req.body.verified;
    req.user.developer = req.body.developer === undefined
      ? req.user.developer
      : req.body.developer;
    req.user.tier = req.body.tier === undefined
      ? req.user.tier
      : req.body.tier;
    req.user.premium = req.body.premium === undefined
      ? req.user.premium
      : req.body.premium;

    req.user.xp = req.body.xp === undefined
      ? req.user.xp
      : req.body.xp;
    req.user.shits = req.body.shits === undefined
      ? req.user.shits
      : req.body.shits;
    req.user.trophies = req.body.trophies === undefined
      ? req.user.trophies
      : req.body.trophies;

    req.user.likes = req.body.likes === undefined
      ? req.user.likes
      : req.body.likes;
  }
